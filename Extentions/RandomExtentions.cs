...
public static class RandomExtentions
{
    ...
    public static T Choice<T>(this Rand random, params T[] choices)
    {
        if (choices.IsEmpty())
            return default;

        return choices[random.Integer() % choices.Length];
    }
    ...
    public static BoxColor RandomColor(this Rand random)
        => new(random.Integer(0, 255), random.Integer(0, 255), random.Integer(0, 255));
    ...
    public static Vect2 RandomVector(this Rand random) => new(random.Float(-1, 1), random.Float(-1, 1));
    ...
    public static Vect2 RandomDirection(this Rand random)
    ...
    public static Vect2 RandomVector(this Rand random, float minimum, float maximum)
    {
        if (minimum > maximum)
            throw new ArgumentException("Minimum value must be less than or equal to maximum value.");

        return new(random.Float(minimum, maximum), random.Float(minimum, maximum));
    }
}
